  ...
  const extractCompanyDetails = (companiesArr) => {
    const detailsArr = companiesArr.map((cmpy, i) => {
      ...
      const values = [
        cmpy.IdAzienda,
        cmpy.DesAzienda,
        props.DesQuestionariArr[i].DesQuestionario,
    ...
    <React.Fragment>
      ...
      <HeaderTable rows={props.headerText} sendStyles={props.sizeArr} />
      {
        extractCompanyDetails(props.companies).map((company, i) => {
          return (
            <RowsTable key={i} rows={company} sendStyles={props.sizeArr} />
          );
        })
      ...
      {IsVisible && (
        <CreateCompany
          ...
          companyData={{
            IdAzienda: props.companies.at(-1).IdAzienda + 10, //Takes last id and adds 1 to set the new id
